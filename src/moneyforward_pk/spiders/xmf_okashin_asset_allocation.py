"""岡崎信用金庫 (okashin.x.moneyforward.com) 派生 Moneyforward の資産配分スパイダー."""
...
class XmfOkashinAssetAllocationSpider(XMoneyforwardLoginMixin, MfAssetAllocationSpider):
    """``xmf_okashin`` variant 用 asset_allocation spider.

    URL/parser は ``MfAssetAllocationSpider`` から継承し、
    ``XMoneyforwardLoginMixin`` で partner-portal のログインフローを上書きする.
    """

    name = "xmf_okashin_asset_allocation"
