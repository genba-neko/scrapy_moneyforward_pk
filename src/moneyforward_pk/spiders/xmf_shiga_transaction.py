"""滋賀銀行 (shiga.x.moneyforward.com) 派生 Moneyforward の取引明細スパイダー."""
...
class XmfShigaTransactionSpider(XMoneyforwardLoginMixin, MfTransactionSpider):
    """``xmf_shiga`` variant 用 transaction spider.

    URL/parser は ``MfTransactionSpider`` から継承し、
    ``XMoneyforwardLoginMixin`` で partner-portal のログインフローを上書きする.
    """

    name = "xmf_shiga_transaction"
