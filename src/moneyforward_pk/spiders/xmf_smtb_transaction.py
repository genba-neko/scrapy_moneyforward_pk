"""三井住友信託銀行 (smtb.x.moneyforward.com) 派生 Moneyforward の取引明細スパイダー."""
...
class XmfSmtbTransactionSpider(XMoneyforwardLoginMixin, MfTransactionSpider):
    """``xmf_smtb`` variant 用 transaction spider.

    URL/parser は ``MfTransactionSpider`` から継承し、
    ``XMoneyforwardLoginMixin`` で partner-portal のログインフローを上書きする.
    """

    name = "xmf_smtb_transaction"
