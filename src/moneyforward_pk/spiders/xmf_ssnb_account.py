"""住信SBIネット銀行 派生 Moneyforward の口座スパイダー."""
...
class XmfSsnbAccountSpider(XMoneyforwardLoginMixin, MfAccountSpider):
    """``xmf_ssnb`` variant 用 account spider.

    URL/parser は ``MfAccountSpider`` から継承し、
    ``XMoneyforwardLoginMixin`` で partner-portal のログインフローを上書きする.
    """

    name = "xmf_ssnb_account"
