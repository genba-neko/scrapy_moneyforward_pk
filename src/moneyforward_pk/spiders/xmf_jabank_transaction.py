"""JAバンク (jabank.x.moneyforward.com) 派生 Moneyforward の取引明細スパイダー."""
...
class XmfJabankTransactionSpider(XMoneyforwardLoginMixin, MfTransactionSpider):
    """``xmf_jabank`` variant 用 transaction spider.

    URL/parser は ``MfTransactionSpider`` から継承し、
    ``XMoneyforwardLoginMixin`` で partner-portal のログインフローを上書きする.
    """

    name = "xmf_jabank_transaction"
