"""JAバンク (jabank.x.moneyforward.com) 派生 Moneyforward の資産配分スパイダー."""
...
class XmfJabankAssetAllocationSpider(XMoneyforwardLoginMixin, MfAssetAllocationSpider):
    """``xmf_jabank`` variant 用 asset_allocation spider.

    URL/parser は ``MfAssetAllocationSpider`` から継承し、
    ``XMoneyforwardLoginMixin`` で partner-portal のログインフローを上書きする.
    """

    name = "xmf_jabank_asset_allocation"
