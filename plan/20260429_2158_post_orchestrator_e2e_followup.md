...
## C. transaction 月切替 timeout (2025/10 で発生、データ欠損)
...
| C 副次 | silent skip 検知 | **部分対応 (staged)** (PlaywrightError 検知追加) | 未 commit |
| C 本体 | 月切替 click intercept | **未着手** | — |
| D | typed run で他種別 JSON 上書き | **対応済** | 2a837a7 |

### 残 task

- [ ] C 本体: `spiders/transaction.py::parse_month` の月クリックを `:visible`
      pseudo + `wait_for(state="visible")` で再実装 (案 1 推奨)
- [ ] staged 分 (B + C 副次) を commit (lint/pyright 確認後)
- [ ] WSL 経路で E2E 再実行 → orchestrator 完走確認
- [ ] 完走確認後 PR #47 を ready for review に切替

### 完了済 task

- [x] A: WSL 経由実行に切替 (2206ef9)
- [x] B: `summarize()` に `invocations` 引数追加 + test (staged)
- [x] C 副次: `_classify_result` に PlaywrightError 検知 + test 2 件 (staged)
