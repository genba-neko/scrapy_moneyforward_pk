...
## C. transaction 月切替 timeout (2025/10 で発生、データ欠損) — [対応済 commit 47d2df2 + 案 (c)+(a) staged]

### 試行履歴

| 案 | E2E 結果 | 採否 |
|----|---------|------|
| 案 1: `:visible` pseudo + `wait_for(state="visible")` (47d2df2) | 18 中 1 失敗 (xmf_ssnb finance, 2025/10 + 2025/11 欠損) | 部分有効 |
| 案 2: `:visible` + `force=True` (2a75ee9 → reset 済) | 18 中 4 失敗 (悪化) | **却下** |
| 案 (c)+(a): `wait_for_function` (DOM 安定化) + `dispatch_event("click")` | **18 中 0 失敗 / 全 6 transaction で months_fetched=12** | **採用** |

### 案 2 失敗原因

`force=True` は actionability check を skip するが element bbox/座標が無いと
click event を dispatch できず即時 fail。 案 1 の 10s 待機で間に合っていた DOM
安定化を失い、 即時 fail に変わった。 codex 相談で確認。
...
# (c) DOM 安定化: 月一覧再描画完了を offsetParent で確認
...
# locator 再解決 (古い参照排除 + :visible で前年要素回避)
month_li = p.locator(
    f'li[data-year="{year}"][data-month="{month}"]:visible'
).first
# (a) dispatch_event: bbox/scroll 不要、 force=True と異なり座標無くても発火
...
- `{name}/months_failed` カウンタ bump (silent skip 検知)
- `_classify_result` が同カウンタを `failed: PartialMonthFetch` に分類

### test

- `test_parse_month_yields_items_after_switcher_succeeds` (dispatch_event 検証)
- `test_parse_month_aborts_when_switcher_throws` (months_failed bump 検証)
- `test_parse_month_aborts_when_dom_stabilization_times_out` (新規)
...
| C 本体 | 月切替 click intercept | **対応済** (`wait_for_function` + `dispatch_event`) | 47d2df2 + staged |
| D | typed run で他種別 JSON 上書き | **対応済** | 2a837a7 |

### 残 task

- [ ] PR #47 を ready for review → CI 緑確認 → master merge → plan に `[完了 PR#47 YYYY-MM-DD]` 追記
...
- 全 6 transaction invocation で `months_fetched: 12` (12 ヶ月完全取得)
- `Month switcher failed` warning 0 件
- 出力 JSON: transaction 696 / account 41 / asset_allocation 18 件 全 valid
...
- [x] C 本体: `parse_month` を `wait_for_function` + `dispatch_event` (47d2df2 → 案 1 → 案 (c)+(a) で完成)
- [x] D: `initialize/finalize_output_files` に `spider_types` 引数 (2a837a7)
- [x] E: Opus レビュー指摘 cleanup (f0e5330)

---

## E. Opus レビュー指摘 cleanup — [対応済 staged]

PR #47 全体 (origin/master..HEAD 6 commits) に対する Opus レビュー (本ファイル
冒頭テーブルとは独立) で抽出された Critical 2 件 + Major 2 件 + Minor 1 件への
対応。 却下した指摘 (Major 5 / Minor 6, 8-13 / Nit 14-18) の根拠は本セッション
内ログに記録。
...
| 指摘 | 対応 |
|------|------|
| Critical 1: `Invocation.__repr__` 経由の password 平文流出 | `Account` / `Invocation` の `password` を `field(repr=False)` 化 |
| Critical 2: `job_runner.bat` が削除済 spider 名 (`mf_transaction` 等) を直叩き | `wsl_runner.bat` 委譲型に書き換え (Issue #40 後の正路 = WSL 経由に統一) |
| Major 3: `_login_flow_partner_portal` dead method | 削除 + 関連 docstring (モジュール先頭 / `XMoneyforwardLoginMixin`) を「フォーム auto-detect」記述に修正 |
| Major 4: `SITE_LOGIN_ALT_*` 系 死コード | `_resolve_credentials` 削除、 `login_alt_user` / `login_alt_pass` フィールド削除、 `login_attempt` パラメータと `meta["moneyforward_login_attempt"]` を全箇所から除去 |
| Minor 7: `CrawlerRunner` 構築失敗時メッセージ捨て | `logger.exception(...)` で詳細を stderr に残す |

### 検証

- ruff: pass / pyright: 0 errors / pytest: 244 passed
- coverage: 84% (死コード削除で 83% → 84%)
- `repr(Invocation(...))` に password 含まれない動作確認済
